...
void GetTopLevel(wl_client* client, wl_resource* resource, uint32_t id) {
  ...
  surface->set_xdg_toplevel(
      std::make_unique<MockXdgTopLevel>(xdg_toplevel_resource, &xdg_toplevel_interface));
}
...
void GetXdgPopup(struct wl_client* client,
  ...
  if (mock_xdg_popup->size().IsEmpty() ||
    ...
    wl_resource_post_error(resource, XDG_SHELL_ERROR_INVALID_POSITIONER,
...
const struct xdg_surface_interface kMockXdgToplevelImpl = {
    &DestroyResource,  // destroy
    nullptr,           // set_parent
    &SetTitle,         // set_title
    &SetAppId,         // set_app_id
